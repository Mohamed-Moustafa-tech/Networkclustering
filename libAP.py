...
class LSOprimizer:
    def __init__(self, GE, G, pat, L_min, L_max, T = 20, max_iter=100, plot=True, opt_pat=None, k=2,
        ...
        self.ge = GE
        self.genes = list(self.G.nodes())# added .nodes instead of get_vertices
        self.patients = len(pat) #np.array(list(GE.columns))
    ...
    def run_ls(self):
        ...
        if self.seed is None:
            ...
            no_type = True
            while no_type:
                nodes = self.dfs(self.G.nodes(np.random.choice(self.genes, 1)[0]), self.init_size) #changed vertix to node
